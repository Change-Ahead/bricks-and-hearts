...
public class LandlordServiceTests : IClassFixture<TestDatabaseFixture>
{
    ...
    [Fact]
    public async void LinkExistingLandlordWithUser_WitValidLink_ReturnsErrorUserAlreadyHasLandlordRecord()
    {
        // Arrange
        await using var context = Fixture.CreateReadContext();
        var service = new LandlordService(context);
        var inviteLink = context.Landlords.Single(l => l.FirstName == "UnlinkedLandlord").InviteLink;
        if (inviteLink == null)
        {
            throw new Exception("Expect invite link to exist!");
        }
        var landlordUserModel = context.Users.Single(u => u.GoogleUserName == "LandlordUser");
        var nonAdminUser = new BricksAndHeartsUser(landlordUserModel, new List<Claim>(), "google");
