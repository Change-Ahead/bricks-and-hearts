...
public class CsvImportService : ICsvImportService
{
    ...
    public CsvImportService(BricksAndHeartsDbContext dbContext, IPostcodeService postcodeService,
    ...
    public async Task<(List<string> FlashTypes, List<string> FlashMessages)> ImportTenants(IFormFile csvFile)
    {
        ...
        {
            foreach (var tenant in _dbContext.Tenants) _dbContext.Tenants.Remove(tenant);
        ...
        int lineNo)
    {
        ...
        if (tenant.Name == null)
        {
            ...
            flashTypes.Add("danger");
            flashMessages.Add(
                $"Name is missing from record on line {lineNo} (email address provided is {tenant.Email}). This record has not been added to the database. Please add a name to this tenant in order to import their information.");
            return (flashTypes, flashMessages);
        }
    ...
    private bool? CheckBoolInput(string propName, string? propValue, string tenantName, ICollection<string> flashTypes,
        ICollection<string> flashMessages)
    {
    ...
    private string InvalidInputMessage(string tenantName, string propName, string propValue)
    {
        ...
        return
            $"Invalid input in record for tenant {tenantName}: '{propName}' cannot be '{propValue}' as this is the wrong data type. This input has been ignored.";
    }
}